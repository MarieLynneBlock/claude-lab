...
- **Use markdown headers and bullets** — Claude scans structure.
...
<sub>(Enterprise: `allowManagedHooksOnly` in managed settings blocks user/project/plugin hooks, then only admin-defined hooks run.)</sub>
...
Managed policy settings          # Organisation or admin-controlled
```
